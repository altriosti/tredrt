...
const PORTAL_FRAG = /* glsl */`
  ...
  void main() {
    ...
    // Square aperture: a passage is a box, so distance is measured to the
    // four walls, not to a centre point.
    vec2 a = abs(p);
    float box = max(a.x, a.y);

    float edge = smoothstep(uOpen, uOpen - 0.30, box);
    float rim  = smoothstep(uOpen + 0.05, uOpen, box) * smoothstep(uOpen - 0.10, uOpen, box);

    // The four corners burn brightest as the passage tears open.
    float corner = smoothstep(uOpen * 0.55, uOpen, min(a.x, a.y)) * rim;

    // Light filaments travelling along the walls of the frame.
    float along = a.x > a.y ? p.y : p.x;
    float fil = sin(along * 22.0 + uTime * 2.2) * 0.5 + 0.5;
    fil *= sin(along * 7.0 - uTime * 1.1) * 0.5 + 0.5;

    vec3 core = mix(vec3(0.18, 0.42, 0.95), vec3(1.0), pow(1.0 - box / max(uOpen, 0.001), 2.0));
    float alpha = edge * (0.10 + fil * 0.20) + rim * 1.3 + corner * 2.0;
...
export class VoidScene {
  ...
  update(dt, t) {
    ...
    // A box does not spin: it drifts and breathes instead.
    this.portal.rotation.z = Math.sin(t * 0.12) * 0.03;
  }
