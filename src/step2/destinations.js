// -----------------------------------------------------------------------------
// WORLD ARCHETYPES (placeholder environment configuration)
//
// These describe how a destination Earth LOOKS. They deliberately carry no
// planet or universe names: those come only from the supplied CSV, which is the
// authoritative destination database.
//
// This is development configuration until the full planet-generation dataset is
// supplied. Each archetype is a complete world definition in the same data
// format the engine already builds from, so adding categories later needs no
// engine changes.
// -----------------------------------------------------------------------------
...
/** Fixed order, so the deterministic archetype pick stays stable. */
export const ARCHETYPE_ORDER = Object.keys(ARCHETYPES);
